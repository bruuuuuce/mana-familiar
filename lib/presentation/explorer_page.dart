...
class LegacyReadOnlySourceEditor extends StatefulWidget {
  const LegacyReadOnlySourceEditor({
    super.key,
  ...
  @override
  State<LegacyReadOnlySourceEditor> createState() =>
      _LegacyReadOnlySourceEditorState();
}

class _LegacyReadOnlySourceEditorState
    extends State<LegacyReadOnlySourceEditor> {
...
class JourneyStore {
  ...
  Future<SafeFile> _fixtureFile() async {
    ...
    if (fixture == null || fixture.isEmpty) {
      throw StateError('No direct artifact was configured.');
    }
    final file = await SafePathPolicy.resolveDirectFile(fixture);
    if (file == null) throw StateError('Journey artifact does not exist.');
    return file;
  }

  Future<JourneyGraph> _loadFixture() async {
    try {
      return JourneyGraph.decode(await (await _fixtureFile()).readAsString());
    } on SafePathException catch (error) {
      throw StateError('Journey artifact blocked: ${error.message}');
    }
  }
  ...
  Future<List<String>> list() async {
    ...
    if (fixture != null && fixture.isNotEmpty) {
      final graph = await _loadFixture();
  ...
  Future<JourneyGraph> load(String id) async {
    if (!_isSafeJourneyId(id)) {
      throw StateError('Invalid Journey identifier.');
    }
    final fixture = config.fixturePath;
    if (fixture != null && fixture.isNotEmpty) {
      final graph = await _loadFixture();
  ...
  Stream<void> watch(String id) {
    ...
    if (!_isSafeJourneyId(id)) {
      return Stream<void>.error(
        StateError('Journey watcher blocked: invalid Journey identifier.'),
      );
    }
    final target = SafePathPolicy(journeys.path).resolveDirectorySync(id);
    if (target == null) return const Stream<void>.empty();
  ...
  Future<String> loadDiagram(
    ...
    try {
      final fixture = config.fixturePath;
      final asset = fixture != null && fixture.isNotEmpty
          ? await SafePathPolicy(
              (await _fixtureFile()).file.parent.path,
              maxBytes: SafePathPolicy.artifactMaxBytes,
            ).resolveFile(path)
          : await SafePathPolicy(
              journeys.path,
              maxBytes: SafePathPolicy.artifactMaxBytes,
            ).resolveFile('$journey/$path');
      if (asset == null) throw StateError('Diagram asset is missing.');
      return asset.readAsString();
    } on SafePathException catch (error) {
      throw StateError('Diagram asset blocked: ${error.message}');
    }
  }
}
